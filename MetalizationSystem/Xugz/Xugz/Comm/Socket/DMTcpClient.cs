...
namespace Xugz
{
    public class DMTcpClient : Component
    {
        ...
        /// 手动断开连接
        /// </summary>
        public bool ManuDisConnect = false;
        /// <summary>
        /// 服务器IP地址
        /// </summary>
        [Description("服务端IP")]
        ...
        private void CreateConnection(bool delayFlag = false)
        {
            try
            {
                ...
                this.Tcpthread = new Thread(new ThreadStart(this.MinitorConnection)) { IsBackground = true };
                ...
                while (this.IsStartTcpthreading)
                {
                    if (!this.Tcpclient.Connected)
                    {
                        try
                        {
                            ...
                            this.Tcpclient.Connect(ServerIp, this.ServerPort);
