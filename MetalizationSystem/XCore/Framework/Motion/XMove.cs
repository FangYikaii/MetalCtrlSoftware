...
namespace XCore
{
    public class XMove : XCtrlEventHandler
    {
            ...
            if (m_Move == true)
            {
                ...
                for (int i = 0; i < m_Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        if (XDevice.Instance.FindAxisById(m_AxisId[i]).IsFeedback)
                        {
                            if (m_CheckLmt)
                            {
                                if (Math.Abs(XDevice.Instance.FindAxisById(m_AxisId[i]).POS - m_Pos[i]) > 1)
                                {
                                    bDone = false;
                                }
                            }
                ...
                if (bDone)
                {
                    if (m_MoveMode == MoveMode.MOVE_HOME)
                    {
                        ...
                        for (int i = 0; i < m_Count; i++)
                        {
                            if (XDevice.Instance.FindAxisById(m_AxisId[i]).IsASTP == false)
                            {
        ...
        {
            foreach (KeyValuePair<int, XAxis> kvp in XDevice.Instance.AxisMap)
            {
